...
| Tests | 846/846 passing (26 test files) |
| Sessions | 40+ |
| Status | All 16 roadmap items + Legion mechanics + Hero Progression UI (XP spending) complete |
...
- Campaign flow: Setup → Hero Creation → MissionSelect → Combat → PostMission → SocialPhase → repeat. Hero Progression accessed from MissionSelect via UPGRADE HEROES button.
